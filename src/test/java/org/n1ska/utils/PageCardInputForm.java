...
public class PageCardInputForm {
    ...
    public PageInput getCardNoInput() {
    ...
    private void shouldBeAndHaveText(String caption, String text) {
        ArrayList<PageInput> inputs = extractInputControls();
        for (var item : inputs) {
            if (item.getCaption().equalsIgnoreCase(caption)) {
                item.shouldBeAndHaveText(text);
                break;
            }
        }
    }
